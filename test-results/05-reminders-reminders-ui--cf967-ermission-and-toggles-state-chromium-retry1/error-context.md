...
- Name: 05-reminders.spec.ts >> reminders ui >> enable notifications button requests permission and toggles state
- Location: tests/05-reminders.spec.ts:120:7
...
Locator: getByRole('button', { name: '🔔 Enable Notifications' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: '🔔 Enable Notifications' })
...
```ts
  24  | 
  25  |         static async requestPermission(): Promise<NotificationPermission> {
  26  |           FakeNotification.permission = 'granted';
  27  |           return 'granted';
  28  |         }
  29  | 
  30  |         constructor(_title: string, _options?: NotificationOptions) {
  31  |           // No-op for tests.
  32  |         }
  33  |       }
  34  | 
  35  |       // @ts-expect-error Override browser Notification for deterministic tests.
